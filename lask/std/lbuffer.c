...
static int lbuffer_tostring(lua_State *L)
{
	Buffer *buffer = buffer_lcheck(L, 1);
	char buf[LINE_MAX];
	snprintf(buf, sizeof(buf), "buffer (%p, memsiz=%u, datasiz=%u, %s-endian)",
			buffer, (unsigned int)buffer->memsiz, (unsigned int)buffer->datasiz, buffer->be ? "big" : "little");
...
{
	Buffer *buffer = buffer_lcheck(L, 1);
	size_t oldlen = (size_t)luaL_checkinteger(L, 2);
...
static int lbuffer_getb(lua_State *L)
{
	...
	for (size_t i = 0; i < num; i++) {
		lua_pushinteger(L, (lua_Integer)buf->data[i]);
	}
	...
	while (*p != 0 && !empty) {
		...
		case 'c': case 'b':  {
				if (buf->datasiz < 1)  {
					...
					lua_pushinteger(L, (lua_Integer)(uint32)buf->data[0]);
					buffer_shift(buf, 1);
				}
				break;
			}
...
static int lbuffer_fill(lua_State *L)
{
	...
	memset(buffer_grow(buffer, length), (uint8)luaL_checkinteger(L, 2), length);
	lua_pushvalue(L, 1);
	return 1;
}
...
static int lbuffer_putb(lua_State *L)
{
	...
	for (int i = 2; i <= top; i++) {
		uint32 val = (uint32)luaL_checkinteger(L, i);
		*p = (uint8)val;
		p++;
	}
...
#if 0
/*
** self = buffer_putl(l1, ..., lN)
**
** push a serial of unsigned integers into the buffer
*/
static int lbuffer_putl(lua_State *L)
{
	Buffer *buffer = buffer_lcheck(L, 1);
	int top = lua_gettop(L);
	uint8 *p = buffer_safegrow(buffer, (size_t)(top - 1) * 4, L);
...
static int lbuffer_putlist(lua_State *L)
{
	...
	while (*types != 0) {
		char type = *types++;
		uint32 value = (uint32)luaL_checkinteger(L, idx++);
		switch (type) {
		case 'c': case 'b': {
...
/*
** self = buffer:overwrite(offset, u32_or_str)
**
** especially useful when
*/
static int lbuffer_overwrite(lua_State *L)
{
	Buffer *buffer = buffer_lcheck(L, 1);
	ssize_t offset = (ssize_t)luaL_checkinteger(L, 2);
	int type = lua_type(L, 3);

	if (type == LUA_TNUMBER) {
		uint32 val = (uint32)luaL_checkinteger(L, 3);
		if (offset >= 0 && offset + 4 <= (ssize_t)buffer->datasiz) {
			uint8 *p = buffer->data + offset;
			if (buffer->be) {
				uint32_to_bytes_be(val, p);
			} else {
				uint32_to_bytes_le(val, p);
		...
		size_t len;
		const char *str = (const char*)luaL_checklstring(L, 3, &len);
		if (offset >= 0 && offset + len <= (ssize_t)buffer->datasiz)
			memcpy(buffer->data + offset, str, len);
	} else {
		luaL_error(L, "expecting string/integer for argument #3");
	}
...
{
	Buffer *buffer = buffer_lcheck(L, 1);
	size_t nbytes = (size_t)luaL_checkinteger(L, 2);
...
static int lbuffer_shift(lua_State *L)
{
	...
	if (type == LUA_TNUMBER) {
		buffer_shift(buffer, (size_t)luaL_checkinteger(L, 2));
	} else {
...
static int lwriter_putc(lua_State *L)
{
	...
	for (int i = 0; i < argc; i++) {
		uint32 val = (uint32)luaL_checkinteger(L, i + 2);
		*p = (uint8)val;
		p++;
	}
...
/*
** self = writer:putu(i1, i2, ... iN)
*/
static int lwriter_putu(lua_State *L)
{
	Writer *wr = (Writer*)luaL_checkudata(L, 1, WRITER_META);
	int argc = (size_t)lua_gettop(L) - 1;
	int left;
...
static int lwriter_tostring(lua_State *L)
{
	Writer *wr = (Writer*)luaL_checkudata(L, 1, WRITER_META);
	char buf[LINE_MAX];
	snprintf(buf, sizeof(buf), "writer (%p, offset=%u, length=%u, cursor=%u)",
				wr, (unsigned int)wr->offset, (unsigned int)wr->length, (unsigned int)wr->cursor);
	lua_pushstring(L, buf);
	return 1;
}
